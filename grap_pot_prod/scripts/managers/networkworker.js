...
(function() {
  ...
  this.addEventListener("message", function(oEvent) {
    ...
    switch (oEvent.data.t) {
      case "start-layout":
        try {
          resultObj = self.doLayoutSteps(oEvent.data.networkObject, oEvent.data.forceSettings);
        } catch (_error) {
          e = _error;
          postMessage({
            t: "e",
  ...
  self.doLayoutSteps = function(networkObject, forceSettings) {
    var force, i, _i;
    force = d3.layout.force();
    force.size(forceSettings.size);
    force.charge(forceSettings.charge);
    force.linkDistance(forceSettings.linkDistance);
    force.nodes(networkObject.nodes).links(networkObject.relations);
    force.start();
