...
(function() {
  ...
  ObjectUtils = {
    ...
    /*
    parseDate:(str)->
      return moment(str,this.timeFormatArray)._d
    */

    parseDate: function(str) {
      var mnt;
      mnt = moment(str, this.timeFormatArray);
      if (mnt._pf.unusedTokens.length > 0 || mnt._pf.unusedChars > 2 || mnt._pf.score > 4) {
        return null;
      }
      if (mnt.isValid()) {
        return mnt._d;
    ...
    */
    ...
    aggregateInit: function(originalQV, mode, id) {
      var newValue;
      if (mode === "count" || mode === "none" || mode === "first" || mode === "last") {
        return {
          "value": originalQV,
          "meta": {}
        };
      } else if (mode === "add" || mode === "mean" || mode === "max" || mode === "min") {
        if ((originalQV.value == null) || _.isNaN(originalQV.value)) {
          originalQV.value = 0;
        }
        return {
          "value": originalQV,
    ...
    aggregateStep: function(cached, newValue, mode) {
      ...
      if (mode === "add") {
        ...
        cached.meta.count += 1;
        if ((newValue == null) || _.isNaN(newValue)) {
          newValue = 0;
        }
    ...
    aggregateEnd: function(cached, mode) {
      if (mode === "none" || mode === "max" || mode === "min" || mode === "first" || mode === "last") {
        return cached.value;
      } else if (mode === "add") {
        if ((cached.value.value == null) || _.isNaN(cached.value.value)) {
          cached.value.value = 0;
        }
        cached.value.value = parseFloat(Number(cached.value.value).toPrecision(4));
        return cached.value;
      } else if (mode === "mean") {
        if ((cached.value.value == null) || _.isNaN(cached.value.value)) {
          cached.value.value = 0;
        ...
        if (cached.meta && cached.meta.count) {
          ...
          cached.value.value = parseFloat(cached.value.value.toPrecision(4));
          return cached.value;
        } else {
          return cached.value;
        }
    ...
    addValues: function(a, b) {
      ...
      if (type === "String") {
        ...
        if ((a == null) || a === "-" || _.isNaN(Number(a))) {
          a = 0;
        }
        if ((b == null) || b === "-" || _.isNaN(Number(b))) {
          b = 0;
        }
...
(function() {
  ...
  this.addEventListener("message", function(oEvent) {
    ...
    switch (oEvent.data.t) {
      ...
      case "isReady":
        if ((oEvent.data.d != null) && (oEvent.data.d.timeFormatArray != null)) {
          ObjectUtils.timeFormatArray = oEvent.data.d.timeFormatArray;
        } else {
          ObjectUtils.timeFormatArray = ["DD-MM-YYYY", "DD-MM-YYYY hh:mm:ss", "YYYY", "hh:mm:ss", "hh:mm"];
        }
        return postMessage({
          t: "ready"
        });
